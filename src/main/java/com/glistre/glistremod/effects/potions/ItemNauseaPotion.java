...
public class ItemNauseaPotion extends Item {
	...
	private int color;
	private NauseaEffect[] effects;


	
	public ItemNauseaPotion(int maxStackSize, String name, NauseaEffect[] vomitusEffects, int color) {
...
//			player.removePotionEffect(ItemRegistry.vomitusPotion.id);
			...
			player.removePotionEffect(ItemRegistry.poison_protect_potion.id);
	...
	//	    			player.removePotionEffect(ItemRegistry.nauseaProtectPotion.id);
					player.addPotionEffect(new PotionEffect(ItemRegistry.vomitus_potion.id, 300, 0, true, true));
				}
