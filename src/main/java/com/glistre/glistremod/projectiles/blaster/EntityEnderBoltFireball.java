...
public class EntityEnderBoltFireball extends Entity implements IEntityAdditionalSpawnData
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
...
//    private static final String __OBFID = "CL_00001715";
    ...
    public EntityEnderBoltFireball(World worldIn)
    ...
    public EntityEnderBoltFireball(World worldIn, double x, double y, double z)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
...
//        this.getYOffset();
    ...
    public EntityEnderBoltFireball(World worldIn, EntityLivingBase entityLiving, EntityLivingBase entityLIvingB, float float0, float float1)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        ...
        double d0 = entityLIvingB.posX - entityLiving.posX;
        double d1 = entityLIvingB.getEntityBoundingBox().minY + (double)(entityLIvingB.height / 3.0F) - this.posY;
        double d2 = entityLIvingB.posZ - entityLiving.posZ;
 ...
 //           this.getYOffset();
    ...
    public EntityEnderBoltFireball(World worldIn, EntityLivingBase entityliving, float f0)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = entityliving;
...
//        this.getYOffset();
        ...
        this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, f0 * 1.5F, 1.0F);
    }
     ...
     * posY, posZ, yaw, pitch
     */
//  func_180426_a replaced  #setPositionAndRotation2 in 1.8 update
   @Override
   @SideOnly(Side.CLIENT)
   public void func_180426_a(double x, double y, double z, float floatYaw, float floatPitch, int pofRotationIncrements, boolean isTeleport)
   {
       this.setPosition(x, y, z);
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            {
                ++this.ticksInGround;
              //changed 1200 to 1 in next line to try to remove arrow
...
//creates explosion on entity hit
 ...
 //                       this.playSound("random.explosion1", 2.1F, 4.2F);
            ...
            if (this.getIsCritical())
            {
                for (i = 0; i < 4; ++i)
                {
                   	//change from "crit" (arrow smoke) to ""  to remove particle effect or critMagic (blue), witchMagic (heavy purple)  or smoke or bubble or fireworksSpark (white X's)
                    this.worldObj.spawnParticle(EnumParticleTypes.CRIT_MAGIC, this.posX + this.motionX * (double)i / 4.0D, this.posY + this.motionY * (double)i / 4.0D, this.posZ + this.motionZ * (double)i / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ, new int[0]);
 ...
 //               	this.worldObj.spawnParticle(EnumParticleTypes.CRIT, this.posX + this.motionX * (double)i / 4.0D, this.posY + this.motionY * (double)i / 4.0D, this.posZ + this.motionZ * (double)i / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ, new int[0]);
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn)
    {
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0)
        {
            ...
            if (this.canBePickedUp == 1 && !entityIn.inventory.addItemStackToInventory(new ItemStack(GlistreEntityRegistry.blaster_bolt_1, 1)))
            {
                flag = false;
            }
