...
public class TeleporterTobyKing extends Teleporter
{
    ...
    public TeleporterTobyKing(WorldServer worldIn)
    {
    	super(worldIn);
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? BlockRegistry.silver_block_1.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
        ...
        {
            TeleporterTobyKing.PortalPosition portalposition = (TeleporterTobyKing.PortalPosition)this.destinationCoordinateCache.getValueByKey(k);
            d0 = 0.0D;
            ...
            for (int l = -128; l <= 128; ++l)
            {
                ...
                for (int i1 = -128; i1 <= 128; ++i1)
                {
                    for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                    {
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos).getBlock() == BlockRegistry.light_toby_king_portal)
                        {
                            while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == BlockRegistry.light_toby_king_portal)
                            {
                                blockpos = blockpos1;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag1)
            {
                this.destinationCoordinateCache.add(k, new TeleporterTobyKing.PortalPosition((BlockPos)object, this.worldServerInstance.getTotalWorldTime()));
            ...
            if (this.worldServerInstance.getBlockState(((BlockPos)object).west()).getBlock() == BlockRegistry.light_toby_king_portal)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).east()).getBlock() == BlockRegistry.light_toby_king_portal)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).north()).getBlock() == BlockRegistry.light_toby_king_portal)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).south()).getBlock() == BlockRegistry.light_toby_king_portal)
            {
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? BlockRegistry.silver_block_1.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = BlockRegistry.light_toby_king_portal.getDefaultState().withProperty(BlockPortal.AXIS, l5 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? BlockRegistry.silver_block_1.getDefaultState() : iblockstate, 2);
                }
            }
    ...
    public void removeStalePortalLocations(long p_85189_1_)
    {
        if (p_85189_1_ % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                TeleporterTobyKing.PortalPosition portalposition = (TeleporterTobyKing.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
